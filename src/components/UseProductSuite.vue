<script setup></script>
<template>
  <div class="flex place-content-center mt-[140px] px-5">
    <div class="">
      <div class="flex xl:flex-row flex-col gap-10 items-center justify-between">
        <div
          data-aos="fade-up"
          data-aos-delay="100"
          class="text-[56px] font-[700] max-w-[400px] xl:text-start text-center leading-[56px]"
        >
          All-in-one Payments Product Suite
        </div>
        <div class="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-[48px]">
          <div
            data-aos="fade-up"
            data-aos-delay="100"
            class="md:max-w-[320px] w-full px-[32px] py-[32px] flex flex-col gap-3 bg-[#DDEDDE] rounded-[24px]"
          >
            <div class="text-[20px] font-bold">Global Payments</div>
            <div class="">
              Single API interface for accessing 50+ payment processors and 200+ payment methods
            </div>
            <div class=""><img src="/Group17.svg" alt="" /></div>
          </div>
          <div
            data-aos="fade-up"
            data-aos-delay="300"
            class="md:max-w-[320px] w-full px-[32px] py-[32px] flex flex-col gap-3 bg-[#DDEDDE] rounded-[24px]"
          >
            <div class="text-[20px] font-bold">Unified and Native Checkout</div>
            <div class="">Cross-platform, native and seamless payment experience</div>
            <div class=""><img src="/Group18.svg" alt="" /></div>
          </div>
          <div
            data-aos="fade-up"
            data-aos-delay="500"
            class="md:max-w-[320px] w-full px-[32px] py-[32px] flex flex-col gap-3 bg-[#DDEDDE] rounded-[24px]"
          >
            <div class="text-[20px] font-bold">Control Center</div>
            <div class="">
              Management interface for payment analytics, refunds, chargebacks and reconcillation
            </div>
          </div>

          <div
            data-aos="fade-up"
            data-aos-delay="700"
            class="md:max-w-[320px] w-full px-[32px] py-[32px] flex flex-col bg-[#DDEDDE] rounded-[24px]"
          >
            <div class=""><img src="/Group19.svg" alt="" /></div>
            <div class="text-[20px] font-bold">Intelligent payment routing</div>
            <div class="pt-3">
              Automatically chooses the best PSP to dynamically improve auth rates
            </div>
          </div>
          <div
            data-aos="fade-up"
            data-aos-delay="900"
            class="md:max-w-[320px] w-full px-[32px] py-[32px] flex flex-col gap-3 bg-[#DDEDDE] rounded-[24px]"
          >
            <div class="text-[20px] font-bold">Payouts</div>
            <div class="">Unified API interface to optimize speed and cost for global payouts</div>
            <div class=""><img src="/Group20.svg" alt="" /></div>
          </div>
          <div
            data-aos="fade-up"
            data-aos-delay="1100"
            class="md:max-w-[320px] w-full px-[32px] py-[32px] flex flex-col gap-3 bg-[#DDEDDE] rounded-[24px]"
          >
            <div class="text-[20px] font-bold">Compliance-in-a-box</div>
            <div class="flex flex-col gap-4">
              <div class="">. PCI L1 compliant</div>
              <div class="">. GDPR certified</div>
              <div class="">. PCI SSS certified</div>
            </div>
            <div class=""><img src="/Group21.svg" alt="" /></div>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>
